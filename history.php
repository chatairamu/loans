<?php
// history.php
// Page to display the history of all individual payments.
...
<head>
    ...
    <title>Payment History - Payments Dashboard</title>
        ...
        <header class="mb-8">
            ...
            <nav class="bg-white p-3 rounded-lg shadow-md">
                <ul class="flex items-center gap-x-6 text-sm sm:text-base">
                    ...
                    <li><a href="history.php" class="text-indigo-600 font-bold border-b-2 border-indigo-600 pb-2">Payment History</a></li>
        ...
        <main>
            ...
            <div class="mb-6">
                <form id="filter-form" class="flex flex-wrap items-end gap-4 p-4 bg-gray-100 rounded-lg">
                    <!-- Date Range -->
                    <div>
                        <label for="filter_start_date" class="block text-sm font-medium text-gray-700">From</label>
                        <input type="date" name="filter_start_date" id="filter_start_date" class="mt-1 block w-full pl-3 pr-2 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                    </div>
                    <div>
                        <label for="filter_end_date" class="block text-sm font-medium text-gray-700">To</label>
                        <input type="date" name="filter_end_date" id="filter_end_date" class="mt-1 block w-full pl-3 pr-2 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                    </div>
                    <!-- Payment Type -->
                    <div>
            ...
            <!-- History Table -->
            <div id="history-container" class="bg-white rounded-lg shadow-md">
                <div id="loader" class="loader" style="display: none;"></div>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Payment Name</th>
                ...
                <p id="no-history-message" class="hidden text-center text-gray-500 py-8">No payment history found.</p>
                <div id="pagination-container" class="px-6 py-4 border-t flex justify-between items-center"></div>
            </div>
        </main>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const fetchHistory = async () => {
            ...
            const params = new URLSearchParams({
                action: 'get_history',
                page: currentPage,
                filter_type: filterForm.filter_type.value,
                filter_status: filterForm.filter_status.value,
                filter_start_date: filterForm.filter_start_date.value,
                filter_end_date: filterForm.filter_end_date.value
            });
            ...
            try {
                ...
                if (result.status === 'success') {
                    ...
                    tableBody.innerHTML = `<tr><td colspan="5" class="text-center text-red-500 py-4">${result.message}</td></tr>`;
                }
            } catch (error) {
                tableBody.innerHTML = `<tr><td colspan="5" class="text-center text-red-500 py-4">An error occurred: ${error.message}</td></tr>`;
            } finally {
            ...
            history.forEach(item => {
                ...
                content += `
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${formatDateTime(item.paid_on)}</td>
                    </tr>
                `;
            });
        ...
        const renderPagination = (pagination) => {
            ...
            let content = `<div class="flex-1 text-sm text-gray-700">Showing <span class="font-medium">${((current_page - 1) * 15) + 1}</span> to <span class="font-medium">${Math.min(current_page * 15, total_items)}</span> of <span class="font-medium">${total_items}</span> results</div>`;
            content += '<div><nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px">';

            const prevDisabled = current_page === 1 ? 'disabled' : '';
            content += `<button data-page="${current_page - 1}" class="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50" ${prevDisabled}>Prev</button>`;
            ...
            content += `<button data-page="${current_page + 1}" class="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50" ${nextDisabled}>Next</button>`;
